...
//   4. GET/PUT /presets — shared storage for the order tool's saved
//      suppliers and store/factory addresses, so every device that opens
//      the tool sees the same list instead of each device keeping its own
//      separate copy. Same JSON-file-on-disk pattern as tokens.json below.
...
// ── CORS helper ──
// Shared by /submit-order and /presets: both are called directly by the
// order tool's own JavaScript (a plain fetch, not routed through
// ServiceM8), and the tool can be opened either through this backend's own
// /order-tool route or as a standalone downloaded copy of the HTML file on
// a completely different origin — so both routes need this rather than
// relying on same-origin requests.
...
// ── Step 5: shared presets (saved suppliers + store/factory addresses) ──
// GET returns the current shared list; PUT replaces it wholesale with
// whatever the order tool currently has after a save/remove. One JSON file
// on disk, same pattern as tokens.json above — persists across restarts
// only if PRESETS_FILE's directory (e.g. /data) is a mounted persistent
// Volume, same caveat as tokens.
...
app.options('/presets', cors);
app.get('/presets', cors, (req, res) => {
  res.json(loadPresets());
});
app.put('/presets', cors, (req, res) => {
  const { suppliers, stores } = req.body || {};
  if (!Array.isArray(suppliers) || !Array.isArray(stores)) {
    return res.status(400).json({ success: false, error: 'Body must be { suppliers: [...], stores: [...] }' });
  }
  savePresets({ suppliers, stores });
